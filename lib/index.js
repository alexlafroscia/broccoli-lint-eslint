...
EslintValidationFilter.prototype.processString = function processString(content, relativePath) {
  ...
  // verify file content
  const pathToFile = this.eslintrc + '/' + relativePath;
  const configForFile = this.cli.getConfigForFile(pathToFile);


  if (!configForFile.globals || !Array.isArray(configForFile.globals)) {
    configForFile.globals = [];
  }
  // this.cli.options = configForFile;

  console.info(`Path to file: ${pathToFile}`);
  console.info(`Config for file: ${JSON.stringify(configForFile, null, 4)}`);

  const output = this.cli.executeOnText(content, pathToFile);
